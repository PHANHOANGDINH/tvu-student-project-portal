...
export async function lecturerList(req,res){try{return send(res,await service.lecturerList(req.user.id,req.query))}catch(e){console.error(e);return sendError(res,{statusCode:500,message:'Không thể tải lớp phụ trách'})}}
export async function lecturerDetail(req,res){try{return send(res,await service.lecturerDetail(req.user.id,req.params.id))}catch(e){console.error(e);return sendError(res,{statusCode:500,message:'Không thể tải không gian lớp'})}}
