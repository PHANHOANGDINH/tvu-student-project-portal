...
const cfg={
 ...
 courseClasses:{required:['SubjectId','SemesterId','LecturerId','Status'],fields:['SubjectId','SemesterId','LecturerId','MaxStudents','Status','AllowSelfEnrollment','EnrollmentOpenAt','EnrollmentCloseAt']}
}
...
export async function list(entity,q){if(!cfg[entity])return result(false,404,'Không tìm thấy tài nguyên');const p=page(q),r=await repo.listEntities(entity,{...p,academicYearId:q.academicYearId,semesterId:q.semesterId,lecturerId:q.lecturerId,status:String(q.status||'').trim().toUpperCase()});return result(true,200,'Lấy danh sách thành công',{items:r.items,page:p.page,pageSize:p.pageSize,totalItems:r.total,totalPages:Math.ceil(r.total/p.pageSize)})}
...
export async function lecturerList(uid,q){const p=page(q),r=await repo.listLecturerClasses(uid,{...p,status:String(q.status||'').trim().toUpperCase()});return result(true,200,'Lấy danh sách lớp giảng dạy thành công',{items:r.items,page:p.page,pageSize:p.pageSize,totalItems:r.total,totalPages:Math.ceil(r.total/p.pageSize)})}
export async function lecturerDetail(uid,id){const key=number(id);if(!key)return result(false,400,'Id không hợp lệ');const item=await repo.findLecturerClass(uid,key);return item?result(true,200,'Lấy chi tiết lớp thành công',item):result(false,404,'Không tìm thấy lớp hoặc giảng viên không giảng dạy lớp')}
