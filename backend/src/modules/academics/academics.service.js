...
export async function lecturerDetail(uid,id){const key=number(id);if(!key)return result(false,400,'Id không hợp lệ');const item=await repo.findLecturerClass(uid,key);return item?result(true,200,'Lấy không gian lớp thành công',item):result(false,404,'Không tìm thấy lớp hoặc giảng viên không được phân công')}
