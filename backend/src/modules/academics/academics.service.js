...
const page=q=>({page:number(q.page)||1,pageSize:Math.min(number(q.pageSize||q.limit)||10,100),search:String(q.search||'').trim(),academicYearId:number(q.academicYearId,true),semesterId:number(q.semesterId,true),subjectId:number(q.subjectId,true),status:String(q.status||'').trim().toUpperCase()||null})
...
export async function studentDetail(uid,id){const key=number(id);if(!key)return result(false,400,'Id không hợp lệ');if(!await repo.findEntity('courseClasses',key))return result(false,404,'Không tìm thấy lớp học phần');const item=await repo.findStudentClass(uid,key);return item?result(true,200,'Lấy chi tiết lớp thành công',item):result(false,403,'Sinh viên chưa được ghi danh vào lớp học phần này')}
export async function lecturerList(uid,q){const p=page(q),r=await repo.listLecturerClasses(uid,p);return result(true,200,'Lấy danh sách lớp phụ trách thành công',{items:r.items,page:p.page,pageSize:p.pageSize,totalItems:r.total,totalPages:Math.ceil(r.total/p.pageSize)})}
export async function lecturerDetail(uid,id){const key=number(id);if(!key)return result(false,400,'Id không hợp lệ');if(!await repo.findEntity('courseClasses',key))return result(false,404,'Không tìm thấy lớp học phần');const item=await repo.findLecturerClass(uid,key);if(!item)return result(false,403,'Giảng viên chưa được phân công lớp học phần này');item.students=await repo.listLecturerClassStudents(uid,key);return result(true,200,'Lấy không gian lớp thành công',item)}
