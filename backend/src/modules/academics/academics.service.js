...
export async function lecturerDetail(uid,id){const key=number(id);if(!key)return result(false,400,'Id không hợp lệ');if(!await repo.findEntity('courseClasses',key))return result(false,404,'Không tìm thấy lớp học phần');const item=await repo.findLecturerClass(uid,key);return item?result(true,200,'Lấy không gian lớp thành công',item):result(false,403,'Giảng viên chưa được phân công lớp học phần này')}
