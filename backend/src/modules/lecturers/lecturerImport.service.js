...
export async function confirm(data,adminId){cleanExpired();if(data?.mode!=='atomic')return fail(400,'Chỉ hỗ trợ chế độ toàn bộ hoặc không có gì.');const item=previews.get(String(data?.previewId||''));if(!item)return fail(410,'Bản xem trước đã hết hạn hoặc không tồn tại.');if(item.adminId!==Number(adminId))return fail(403,'Bản xem trước không thuộc tài khoản hiện tại.');if(item.invalidCount)return fail(409,'File còn dòng lỗi nên chưa thể nhập.');const rows=await Promise.all(item.rows.map(async row=>({...row,passwordHash:await hashPassword(row.password)})));try{const created=await repository.importLecturers(rows);previews.delete(String(data.previewId));return ok({createdCount:created.length,lecturers:created},'Nhập danh sách giảng viên thành công.',201);}catch(error){if([2601,2627].includes(error.number))return fail(409,'Dữ liệu đã tồn tại sau khi preview; toàn bộ transaction đã rollback.');return fail(error.statusCode||500,error.statusCode?'Nhập thất bại, toàn bộ transaction đã rollback.':'Không thể nhập giảng viên.');}}
