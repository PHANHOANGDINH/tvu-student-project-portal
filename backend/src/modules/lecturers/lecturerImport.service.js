...
const loadRepository = () => import('./lecturerImport.repository.js');
...
export async function preview(file,adminId,dependencies={}){
  ...
  const repository=dependencies.repository||await loadRepository();const existing=await repository.findExisting(rows.filter(row=>row.email&&row.lecturerCode)),emails=new Set(existing.map(item=>item.email.toLowerCase())),codes=new Set(existing.map(item=>item.lecturerCode?.toUpperCase()));rows.forEach(row=>{if(emails.has(row.email))row.errors.push('Email đã tồn tại trong hệ thống.');if(codes.has(row.lecturerCode))row.errors.push('Mã giảng viên đã tồn tại trong hệ thống.');});
...
export async function confirm(data,adminId){cleanExpired();if(data?.mode!=='atomic')return fail(400,'Chỉ hỗ trợ chế độ toàn bộ hoặc không có gì.');const item=previews.get(String(data?.previewId||''));if(!item)return fail(410,'Bản xem trước đã hết hạn hoặc không tồn tại.');if(item.adminId!==Number(adminId))return fail(403,'Bản xem trước không thuộc tài khoản hiện tại.');if(item.invalidCount)return fail(409,'File còn dòng lỗi nên chưa thể nhập.');const rows=await Promise.all(item.rows.map(async row=>({...row,passwordHash:await hashPassword(row.password)})));try{const repository=await loadRepository();const created=await repository.importLecturers(rows);previews.delete(String(data.previewId));return ok({createdCount:created.length,lecturers:created},'Nhập danh sách giảng viên thành công.',201);}catch(error){return fail(error.statusCode||500,error.statusCode?'Nhập thất bại, toàn bộ transaction đã rollback.':'Không thể nhập giảng viên.');}}
export async function exportCsv(filters={}){const repository=await loadRepository();const rows=await repository.listLecturers(filters);return createCsv(['lecturerCode','fullName','email','faculty','academicDegree','status'],rows.map(row=>[row.lecturerCode,row.fullName,row.email,row.department,row.academicDegree,row.isActive?'ACTIVE':'INACTIVE']));}
