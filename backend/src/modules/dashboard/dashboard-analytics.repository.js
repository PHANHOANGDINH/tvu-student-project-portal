...
export async function studentDataset(name, filters, userId) {
  const request = await requestWithFilters(filters, userId);
  const queries = {
    summary: `SELECT (SELECT COUNT(*) FROM CourseClassEnrollments WHERE StudentId=@Uid AND IsActive=1 AND DeletedAt IS NULL) classes,(SELECT COUNT(*) FROM GroupMembers WHERE StudentId=@Uid AND DeletedAt IS NULL) groups,(SELECT COUNT(*) FROM Notifications WHERE UserId=@Uid AND IsRead=0) newFeedback,(SELECT COUNT(*) FROM Grades g JOIN Submissions s ON s.Id=g.SubmissionId JOIN GroupMembers gm ON gm.GroupId=s.GroupId WHERE gm.StudentId=@Uid AND gm.DeletedAt IS NULL AND g.IsPublished=1) publishedGrades`,
