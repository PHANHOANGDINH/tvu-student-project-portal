...
export default function SubmissionRequirementsPage() {
  ...
  return <div className="student-workflow">
    <div className="page-title"><h2>Yêu cầu nộp bài</h2><p>Xem tất cả yêu cầu, thời gian mở và hạn nộp của các lớp đang tham gia.</p></div>
    ...
    <div className="panel">{loading ? <LoadingState /> : visible.length ? <div className="table-wrap"><table><thead><tr><th>Yêu cầu</th><th>Lớp học phần</th><th>Mở từ</th><th>Hạn nộp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{visible.map(item => <tr key={item.id}><td><strong>{item.title}</strong></td><td>{item.classCode}</td><td>{formatDateTimeVi(item.startAt)}</td><td>{formatDateTimeVi(item.deadline)}</td><td><StatusBadge status={item.effectiveStatus} /></td><td><div className="toolbar-actions"><Link className="btn-light small" to={`/student/submission-requirements/${item.id}`}>Chi tiết</Link>{['OPEN', 'OPEN_LATE'].includes(item.effectiveStatus) && <Link className="btn-primary small" to={`/student/submission-requirements/${item.id}/submit`}>Nộp bài</Link>}</div></td></tr>)}</tbody></table></div> : <EmptyState title="Chưa có yêu cầu nộp bài" description="Yêu cầu của các lớp học phần sẽ xuất hiện tại đây." />}</div>
  </div>
}
