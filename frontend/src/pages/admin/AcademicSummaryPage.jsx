...
import { createAcademic, listAcademic, replaceCourseClassLecturers, setAcademicStatus, updateAcademic } from '../../api/academicsApi'
...
const configs = {
  ...
  'course-classes': { fields: [['code','Mã lớp học phần','text'],['subjectId','Môn học','subjects'],['semesterId','Học kỳ','semesters'],['maxStudents','Sĩ số tối đa','number'],['status','Trạng thái','status']], columns: [['code','Mã lớp'],['subjectId','Môn học'],['semesterId','Học kỳ'],['lecturers','Giảng viên giảng dạy'],['status','Trạng thái']] },
}
...
export default function AcademicSummaryPage({ resource, title }) {
  ...
  const [primaryLecturerId,setPrimaryLecturerId]=useState(''),[coLecturerIds,setCoLecturerIds]=useState([])
  ...
  useEffect(() => {
    ...
    if(key==='course-classes') jobs.push(getUsers({role:'LECTURER',status:'ACTIVE',pageSize:100,sortBy:'fullName',sortOrder:'asc'}).then(response=>['lecturers',normalizeItems(response?.data)]))
  ...
  const displayLecturers=item=>{const lecturers=item?.lecturers||[];return <div className="class-lecturer-list" title={lecturers.map(x=>`${x.fullName} — ${x.assignmentRoleLabel}`).join('\n')}>{lecturers.slice(0,2).map(x=><span key={x.id}><strong>{x.fullName}</strong><small>{x.assignmentRole==='PRIMARY'?'Giảng viên chính':'Phối hợp'}</small></span>)}{lecturers.length>2&&<em>+{lecturers.length-2} giảng viên</em>}{!lecturers.length&&'Chưa phân công'}</div>}
  const display = (item,field) => field==='lecturers' ? displayLecturers(item) : field.endsWith('Date') ? date(item?.[field]) : field==='academicYearId' ? item?.academicYearName||item?.[field] : field==='subjectId' ? item?.subjectName||item?.[field] : field==='semesterId' ? item?.semesterName||item?.[field] : field==='status' ? statusText[item?.[field]]||item?.[field] : item?.[field]??'—'
  const edit = item => { if(!item)return;const next={};config.fields.forEach(([field])=>next[field]=field.endsWith('Date')&&item[field]?String(item[field]).slice(0,10):(item[field]??''));setForm(next);setPrimaryLecturerId(item.lecturers?.find(x=>x.assignmentRole==='PRIMARY'&&x.isActive)?.id||'');setCoLecturerIds(item.lecturers?.filter(x=>x.assignmentRole==='CO_LECTURER'&&x.isActive).map(x=>x.id)||[]);setEditing(item);setOpen(true) }
  const submit = async event => { event.preventDefault();try{setSaving(true);const saved=editing?await updateAcademic(key,editing.id,form):await createAcademic(key,form);if(key==='course-classes'){const classId=editing?.id||saved?.data?.id;const lecturers=[...(primaryLecturerId?[{lecturerId:Number(primaryLecturerId),assignmentRole:'PRIMARY'}]:[]),...coLecturerIds.map(lecturerId=>({lecturerId:Number(lecturerId),assignmentRole:'CO_LECTURER'}))];await replaceCourseClassLecturers(classId,lecturers)};setMessage({type:'success',text:editing?'Cập nhật thành công.':'Thêm mới thành công.'});setOpen(false);setEditing(null);setForm(initial);await load()}catch(error){setMessage({type:'error',text:error.message||'Không thể lưu dữ liệu.'})}finally{setSaving(false)} }
  const closeModal = () => { setOpen(false);setEditing(null);setForm(initial);setPrimaryLecturerId('');setCoLecturerIds([]) }

  return <div className="admin-page"><div className="page-title admin-page-heading"><div><span className="eyebrow">QUẢN LÝ HỌC VỤ</span><h2>{pageTitle}</h2><p>Quản lý thông tin {pageTitle.toLowerCase()} trong hệ thống.</p></div><div className="toolbar-actions">{key==='course-classes'&&<><button className="btn-light" onClick={()=>navigate('/admin/students/import')}><Users size={17}/>Danh sách sinh viên</button><button className="btn-light" onClick={()=>navigate('/admin/course-class-lecturers')}><UserRoundCheck size={17}/>Phân công giảng viên</button></>}<button className="btn-primary" onClick={()=>{setEditing(null);setForm(initial);setPrimaryLecturerId('');setCoLecturerIds([]);setOpen(true)}}><Plus size={18}/>Thêm mới</button></div></div>
