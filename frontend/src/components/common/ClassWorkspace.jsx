...
export default function ClassWorkspace({ course, role, activeTab = '', children }) {
  ...
  return <div className="class-workspace">
    ...
    <nav className="workspace-tabs" aria-label="Không gian lớp học phần">{tabs.map(([label, path]) => { const [pathname, search = ''] = path.split('?'); const selected = pathname === location.pathname && (search ? location.search === `?${search}` : !location.search); return <Link key={label} className={selected ? 'active' : ''} to={path}>{label}</Link> })}</nav>
    {activeTab ? children : <section className="panel workspace-overview"><h3>Không gian dự án theo lớp</h3><p>Mọi nhóm, đề tài, tiến độ, bài nộp và điểm trong khu vực này được giới hạn theo lớp <strong>{course.code}</strong>.</p><div className="workspace-metrics"><div><span>Sinh viên</span><strong>{course.studentCount ?? '—'}</strong></div><div><span>Nhóm</span><strong>{course.groupCount ?? (course.groupId ? 1 : 0)}</strong></div><div><span>Đề tài</span><strong>{course.topicStatus ? 'Đã đăng ký' : 'Chưa đăng ký'}</strong></div><div><span>Trạng thái</span><strong>{course.status === 'ACTIVE' ? 'Đang hoạt động' : 'Đã kết thúc'}</strong></div></div></section>}
