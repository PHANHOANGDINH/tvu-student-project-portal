...
const activityTime = value => value ? new Intl.DateTimeFormat('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric', hour12: false }).format(new Date(value)).replace(' ', ' · ') : ''
export const RecentActivityList = ({ items = [] }) => items.length ? <List className="recent-activity-list" dataSource={items} renderItem={item => <List.Item><List.Item.Meta title={<Typography.Text strong>{item.title}</Typography.Text>} description={`${getRoleLabel(item.status)} · ${activityTime(item.createdAt)}`} /></List.Item>} /> : <AppEmptyState description="Chưa có hoạt động gần đây" />
