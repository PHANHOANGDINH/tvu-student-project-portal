...
BEGIN
  ...
  CREATE UNIQUE INDEX UX_AcademicYears_Name
    ON dbo.AcademicYears(Name)
...
BEGIN
  ...
  CREATE UNIQUE INDEX UX_Semesters_AcademicYear_Code
    ON dbo.Semesters(AcademicYearId, Code)
    WHERE DeletedAt IS NULL;
...
BEGIN
  CREATE TABLE dbo.CourseClasses (
    ...
    Status NVARCHAR(20) NOT NULL CONSTRAINT DF_CourseClasses_Status DEFAULT 'ACTIVE',
    ...
    CONSTRAINT CK_CourseClasses_Status CHECK (Status IN ('ACTIVE', 'INACTIVE', 'COMPLETED', 'CANCELLED')),
    CONSTRAINT CK_CourseClasses_MaxStudents CHECK (MaxStudents IS NULL OR MaxStudents > 0)
  );

  CREATE UNIQUE INDEX UX_CourseClasses_Code
    ON dbo.CourseClasses(Code)
    WHERE DeletedAt IS NULL;
...
IF OBJECT_ID('dbo.CourseClassEnrollments', 'U') IS NULL
BEGIN
  ...
  CREATE UNIQUE INDEX UX_CourseClassEnrollments_Class_Student
    ON dbo.CourseClassEnrollments(CourseClassId, StudentId)
    WHERE DeletedAt IS NULL;
